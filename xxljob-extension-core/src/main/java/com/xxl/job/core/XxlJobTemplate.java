...
import com.xxl.job.core.admin.XxlJobAdminHttpResponse;
import com.xxl.job.core.admin.XxlJobAdminPageListAdapter;
...
public class XxlJobTemplate {
	...
	public ReturnT<String> addUniqueJob(XxlJobInfo jobInfo) {
		...
		if (Objects.isNull(list) || isEmpty(list.getData())) {
			return this.addJob(jobInfo);
		}
		if (jobInfo.getJobDesc() != null) {
			jobInfo.setJobDesc(jobInfo.getJobDesc().trim());
		}
		final String desc = jobInfo.getJobDesc();
		if (list.getData().stream().anyMatch(j -> j.getJobDesc() != null
				&& j.getJobDesc().trim().equals(desc))) {
	...
	/**
	 * 替代 Spring CollectionUtils.isEmpty（避免 core 引入 spring-core 依赖）。
	 */
